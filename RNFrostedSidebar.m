...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors andlabel:(NSArray *)labels{
    if (self = [super init]) {
        ...
        _itemSize = CGSizeMake(_width/2.2, _width/2.2);//侧边栏选项卡大小
        _itemViews = [NSMutableArray array];
        _tintColor = [UIColor colorWithWhite:0.2 alpha:0.7];
        _borderWidth = 2;
        ...
        [_images enumerateObjectsUsingBlock:^(UIImage *image, NSUInteger idx, BOOL *stop) {
            ...
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 41, 54, 12)];
            [label setBackgroundColor:[UIColor clearColor]];
            label.font = [UIFont systemFontOfSize:10.0];
            [label setTextColor:[UIColor whiteColor]];
            label.textAlignment = NSTextAlignmentCenter;
            label.text = [labels objectAtIndex:idx];
...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices {
    return [self initWithImages:images selectedIndices:selectedIndices borderColors :nil andlabel:nil];
}

- (instancetype)initWithImages:(NSArray *)images {
    return [self initWithImages:images selectedIndices:nil borderColors:nil andlabel:nil];
}
...
- (void)didTapItemAtIndex:(NSUInteger)index {
    ...
    if (self.borderColors) {
        ...
        circleShape.strokeColor = stroke.CGColor;//点击时颜色效果
